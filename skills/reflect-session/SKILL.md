...
- **A:**
  ...
  - **Scope личное:** write the rule to the layer file method.md Phase 3 routes to — `_tracked/shared.md` (K1, portable K0) or `_tracked/machines/current/CLAUDE.md` (machine-bound K0). No mirror write: the live `~/.claude/CLAUDE.md` is a thin root that `@import`s the layer files, so the layer edit is visible from the next session. (Legacy setup where `~/.claude/CLAUDE.md` still holds rules directly: edit it in place and move the touched rule into its layer file in the same approval — a one-off migration, not a standing mirror.)
  ...
  - Section placement inside the layer file (method.md Phase 3): K1 → `## Профиль взаимопонимания (К1)`, K0 → `## Дисциплина оркестратора (К0)`. **K2 is two-part:** the full rule body goes to `_tracked/machines/current/k2-environment.md` (lazy file), and one index row (`| триггер | ядро действия |`) goes to the `## Карта среды (К2) — индекс` table in `_tracked/machines/current/CLAUDE.md`. Both K2 writes are one approval — fail one, roll back the other. If the section/index is missing (e.g. workspace CLAUDE.md, or a setup without machine layers), append the full rule at the end without inventing the structure.
...
- **C:** Write the new `SKILL.md` to `~/.claude/local-forks/skills/<name>/SKILL.md`, then `mkdir -p ~/.claude/skills/<name>/` and `ln -sfn ~/.claude/local-forks/skills/<name>/SKILL.md ~/.claude/skills/<name>/SKILL.md` (matches the symlink shape `bootstrap.sh` produces for existing skills). Single source of truth — no second copy to maintain.
- **B:**
